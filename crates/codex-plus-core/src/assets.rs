...
const DREAM_SKIN_RENDERER_REVISION: &str = "19";
...
fn dream_skin_target_runtime_script(settings: &BackendSettings, include_art: bool) -> String {
    ...
    payload = payload
        .replace("  state.observer?.disconnect?.();\n", "")
        .replace("  if (state.timer) clearInterval(state.timer);\n", "")
        .replace("  state.observer = null;\n", "")
        .replace("  state.timer = null;\n", "");
    payload
}
